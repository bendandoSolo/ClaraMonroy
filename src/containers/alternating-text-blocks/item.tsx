...
const AlternatingTextBlock = ({ isEven, title, image, heading, texts }: TProps) => {
    return (
        <li className="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-relative tw-mb-10 lg:tw-mb-[68px] tw-pl-[45px] tw-w-full md:tw-pl-0 last:tw-mb-0">
            ...
            >
                <div className="lg:tw-max-w-[500px]" style={{position: 'relative', top: '50%', transform: 'translateY(-50%)'}}>
                    {title?.content && (

                        <h3 className="tw-uppercase tw-tracking-[3px] tw-text-primary tw-mb-7.5 lg:tw-mb-14 tw-align-bottom">
                            {title.content}
                        </h3>
                    )}
                    {/* image?.src && (
